...
2026-08-13 기준으로, 이 프로젝트를 이해할 때 가장 먼저 알아야 할 계약은 아래와 같습니다.

- 플레이하려면 로그인해야 합니다.
- 로그인 방식은 `Google 로그인` 또는 `게스트(익명) 로그인` 두 가지입니다.
- 여기서 말하는 게스트 로그인도 `Firebase Auth`를 사용합니다.
- 현재 공식 슬롯 저장소는 `Firestore`입니다.
- `slotInstanceId`는 같은 슬롯 번호의 재생성을, `digimonInstanceId`는 해당 슬롯 내 디지몬의 생애를 구분합니다.
- IndexedDB outbox와 활동 로그는 이 식별자로 격리되어 슬롯 삭제·환생 후 이전 생애의 pending 기록이 섞이지 않습니다.
- 새 생애·일반 진화는 Firestore transaction으로 슬롯 상태와 로그를 함께 확정하며, 로컬 조그레스는 `/api/jogress` `complete-local` 서버 transaction이 두 슬롯을 함께 확정합니다.
...
슬롯 삭제 후 같은 번호를 다시 사용해도 `slotInstanceId`가 다르며, 사망·환생으로 새 디지몬이 시작되면 `digimonInstanceId`가 바뀌니다. 새 생애·일반 진화·로컬 조그레스는 revision과 identity를 확인하는 transaction으로 저장하며, 현재 생애의 활동 로그는 최대 50개를 표시합니다.
