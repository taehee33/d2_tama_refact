...
test("디지몬 상태와 카운터를 Discord 중첩 목록으로 조립한다", () => {
  assert.equal(
    formatSlotReportEntry({
      digimonName: "메탈콩알몬",
      slotId: "slot1",
      issues: ["🍖 배고픔", "🔋 기력부족"],
      counters: [
        "🍖 배고픔 0 지속: 경과 8시간 56분 0초 · 남음 3시간 3분 59초\n데드라인 2026. 7. 11. PM 8:48:13\n```text\n7/11 08:48 ███████████████░░░░░ 7/11 20:48\n                   74%\n```",
        "💪 힘 0 지속: 경과 8시간 56분 0초 · 남음 3시간 3분 59초\n데드라인 2026. 7. 11. PM 8:48:13\n```text\n7/11 08:48 ███████████████░░░░░ 7/11 20:48\n                   74%\n```",
        "🩹 부상 누적: 1/15회",
      ],
    }),
    "- **메탈콩알몬** (slot1)\n" +
...
test("상태만·카운터만·정상 슬롯은 빈 계층 없이 조립한다", () => {
  assert.equal(
    formatSlotReportEntry({ digimonName: "상태몬", slotId: "slot1", issues: ["💡 수면호출"] }),
    "- **상태몬** (slot1)\n  - 💡 수면호출"
  );
  assert.equal(
    formatSlotReportEntry({ digimonName: "카운터몬", slotId: "slot2", counters: ["🩹 부상 누적: 1/15회"] }),
    "- **카운터몬** (slot2)\n  - ⏳ **진행 중 카운터**\n    - 🩹 부상 누적: 1/15회"
  );
  assert.equal(
    formatSlotReportEntry({ digimonName: "정상몬", slotId: "slot3" }),
    "- **정상몬** (slot3)"
  );
});
...
test("진행 중 사망·질병 카운터는 냉장고 누적 시간을 제외해 경과·남음·데드라인을 표시한다", () => {
  ...
  assert.match(counters[0], /```text\n7\/11 07:00 ███░{17} 7\/11 19:00\n {19}16%\n```$/);
  assert.match(counters[1], /경과 4시간 0분 0초 · 남음 8시간 0분 0초/);
});

test("일일보고 진행 바는 시작·데드라인 시각 사이에 놓고 퍼센트를 중앙 정렬한다", () => {
  const startedAt = Date.parse("2026-07-23T10:01:08.000Z");
  const nowMs = startedAt + 1 * HOUR_FOR_TEST + 4 * 60_000 + 38_000;
  const counters = resolveActiveDeathDiseaseCounters({
    digimonStats: createRuntimeStats({ fullness: 0, lastHungerZeroAt: startedAt }),
  }, nowMs);

  assert.match(counters[0], /데드라인 2026\. 7\. 24\. AM 7:01:08/);
  assert.match(
    counters[0],
    /```text\n7\/23 19:01 ██░{18} 7\/24 07:01\n {20}8%\n```$/
  );
});
...
test("일일보고는 서버 투영 사망 슬롯을 다른 상태보다 우선해 사망 판정으로 표시한다", async () => {
  ...
  assert.equal(res.statusCode, 200);
  assert.equal(
    res.body.reports[0].messageContent.includes("- **아구몬** (slot1)\n  - 💀 사망 판정"),
    true
  );
