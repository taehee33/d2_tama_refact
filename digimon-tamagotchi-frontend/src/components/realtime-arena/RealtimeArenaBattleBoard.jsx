...
function Fighter({ label, data, hp }) {
  ...
  return (
    <div className="rounded-lg border p-3">
      ...
      <img className="mx-auto my-2 h-16 w-16 object-contain" src={`${data.spriteBasePath || "/images"}/${data.sprite ?? 0}.png`} alt={`${data.digimonName} 모습`} style={{ imageRendering: "pixelated" }} />
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Fighter label={cpuMode ? "나" : "호스트"} data={battle.participants.host} hp={battle.currentHp.host} />
        <Fighter label={cpuMode ? "CPU" : "게스트"} data={battle.participants.guest} hp={battle.currentHp.guest} />
      </div>
      {latest && <p className="rounded bg-slate-100 p-2 text-center text-sm">이전 라운드: {cpuMode ? "나" : "호스트"} {latest.hostDamageTaken} 피해 · {cpuMode ? "CPU" : "게스트"} {latest.guestDamageTaken} 피해</p>}
