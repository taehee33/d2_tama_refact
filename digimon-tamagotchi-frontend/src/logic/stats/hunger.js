 ...
 */
export function handleHungerTick(currentStats, digimonData, deltaSec = 1, isSleeping = false, referenceTimeMs = Date.now()) {
  ...
  if (hungerCycle > 0) {
    ...
    if (s.hungerCountdown <= 0) {
      const zeroBoundaryAt = referenceTimeMs + (s.hungerCountdown * 1000);
      // 정상적으로 배고픔 감소
