...
- 현재 배고픔·힘이 0인 상태와 사망 카운터, 활성 수면 호출, 조그레스 대기 및 사망 우선 판정은 기존대로 유지합니다.

### 테스트 보강
- 종료된 호출 제외, 현재 0 상태와 카운터 유지, 활성 수면 호출 및 사망 우선 판정을 검증했습니다.
...
### 아키텍처 결정 근거
- 케어미스 이력과 저장 계약은 변경하지 않고 일일보고의 현재 상태 표현만 조정해 호출 기록 화면과 긴급 알림 경계를 보존했습니다.
...
### 저장 및 동기화 저장 위치 표시
- 게임 화면의 `저장 및 동기화` 카드에서 슬롯 상태, 재전송, 활동 기록 전송, 오류 항목에 실제 저장 위치를 괄호로 표시하도록 변경했습니다.
- 활동 기록 대기 개수는 `대기 n개 · IndexedDB → Firestore logs` 형식으로 표시해 이 기기 임시보관함과 서버 로그 저장 위치를 구분했습니다.
- 일반 먹이 기록 상태 문구를 `먹이 기록 15분 요약 대기`로 바꿔, 즉시 개별 저장이 아니라 15분 단위 요약 저장임을 명확히 했습니다.

### 영향받은 파일
- `src/components/GameSyncInfo.jsx`
- `src/components/GameSyncInfo.test.jsx`
- `src/components/GameHeaderMeta.test.jsx`
- `docs/REFACTORING_LOG.md`

### 아키텍처 결정 근거
- 저장 계약과 전송 흐름은 그대로 유지하고 표시 라벨만 보강했습니다. 슬롯 본체와 활동 기록은 서로 다른 저장 경계를 사용하므로, 컴포넌트 문구에서 위치를 직접 드러내는 것이 가장 좁은 UX 개선입니다.
...
### 최근 호출 확인 상태와 모두 확인 처리
- 최근 호출/케어미스 기록에 `미확인`/`확인됨` 상태를 추가하고, 최근 탭에서 `모두 확인`으로 현재 보이는 최근 호출을 한 번에 확인 처리할 수 있게 했습니다.
- 확인된 최근 호출 ID는 슬롯 스탯의 `acknowledgedRecentCallIds`에 저장해 새로고침 후에도 최근 호출 알림 버튼이 다시 뜨지 않도록 했습니다.
- 활성 호출이 없고 미확인 최근 호출이 있을 때만 `🕘📣` 버튼을 표시하고, 최근 기록이 모두 확인되면 기록은 남기되 화면 버튼은 숨기도록 했습니다.
...
### 아키텍처 결정 근거
- 최근 호출 기록은 별도 서버 컬렉션이 아니라 슬롯 런타임 상태와 활동 로그에서 만들어지므로, 확인 상태도 슬롯 문서의 작은 ID 목록으로 저장하는 것이 가장 좁은 변경입니다. 개별 기록 삭제 없이 읽음 상태만 분리해 게임 결과와 알림 UX의 책임을 나눴습니다.
...
- 서버가 저장하는 Discord 미연결 reason인 `missing_webhook`을 UI에서 제대로 해석하도록 맞추고, 통합 전 알림처럼 Discord 전송 상태가 알림 문서에 기록되지 않은 항목은 `Discord 기록 없음`으로 표시하도록 정리했습니다.
...
- `not_requested`로 저장된 과거/비대상 Discord 상태가 `Discord 제외`로 오해되지 않고 `Discord 기록 없음`으로 보이는지 검증하는 케이스를 추가했습니다.
...
- 특히 최근 정리된 자유게시판, 버그제보 / QnA, 자랑게시판의 역할 차이와 지원 보드의 이미지 첨부/재현 정보 흐름을 가이드에 반영해, 현재 서비스 구조와 맞지 않던 예전 설명을 걷어 냈습니다.
- 이후 2차 정리에서 카드 나열형 표현을 줄이고, `큰 흐름 + 페이지 역할 + 메뉴 구조 + 접이식 육성 포인트` 중심의 차분한 제품 가이드형 레이아웃으로 다시 단순화했습니다.
- 스타일은 `src/styles/guide.css`로 분리해 서비스 카드 톤은 유지하되, 색상 수를 줄이고 타임라인/아코디언 위주로 시선을 안정시키도록 정리했습니다.

### 테스트 보강
- `src/pages/Guide.test.jsx`를 추가해 새 가이드 페이지가 현재 사이트 동선, 커뮤니티 안내, 플레이 허브 CTA를 정상적으로 렌더링하는지 확인했습니다.

### 영향받은 파일
- `src/pages/Guide.jsx`
- `src/components/guide/GuideOverview.jsx`
- `src/pages/Guide.test.jsx`
- `src/styles/guide.css`
- `docs/REFACTORING_LOG.md`

### 아키텍처 결정 근거
- `DigimonGuidePanel`은 게임 안 모달과 연결된 재사용 패널이라, 서비스 페이지까지 같은 픽셀 패널을 공유하면 문맥이 어색하고 향후 페이지형 확장이 어렵습니다. 그래서 게임 내 패널은 유지하고, 서비스 가이드는 별도 전용 구성으로 분리하는 편이 현재 사이트 구조와 유지보수 모두에 더 안전합니다.
...
### 수면 중 배고픔/기력 긴급 알림 필터 보강
- 서버 긴급 알림 판정에서 `sleepStartAt` 기록 유무와 관계없이 현재 시각이 수면/낮잠 구간이면 배고픔 호출과 기력 호출을 Discord 긴급 알림에서 제외하도록 수정했습니다.
- 앱이 백그라운드였다가 돌아온 직후 저장 큐가 바빠 주기 저장을 한 번 건너뛰면, 보이는 화면에서 30초 뒤 다시 저장을 시도하도록 보강했습니다.

### 테스트 보강
- 수면 구간에 `sleepStartAt`이 누락된 배고픔/기력 호출도 긴급 알림으로 발송되지 않는 회귀 테스트를 추가했습니다.
- 주기 저장 예정 시간이 지난 상태에서 첫 시도가 저장 큐 때문에 건너뛰어도 짧은 재시도로 최신 슬롯 상태를 저장하는 Hook 테스트를 추가했습니다.

### 영향받은 파일
- `api/_lib/urgentCareProjection.js`
- `api/_lib/urgentCareNotifications.test.js`
- `src/hooks/game-runtime/useGamePeriodicSync.js`
- `src/hooks/game-runtime/useGamePeriodicSync.test.js`
- `docs/REFACTORING_LOG.md`
...
### 호출/최근 호출 UI 분리
- 호출 상태 모델에서 `isLogged=true`인 호출을 현재 호출에서 제외하고 최근 호출/케어미스 기록으로 분류하도록 정리했습니다.
- 게임 화면 호출 팝업을 `현재 호출`과 `최근 호출/케어미스 기록` 탭으로 나눴고, 현재 호출이 없지만 최근 기록이 있으면 `🕘📣` 최근 호출 아이콘을 표시하도록 했습니다.
- 수면 조명 경고가 케어미스로 처리된 뒤에도 불이 켜져 있으면 상태 메시지를 `수면 중(불 켜짐 경고! · 케어미스 처리됨)`으로 표시하고, StatsPanel에서는 `케어미스 처리됨 · 불은 아직 켜져 있음`으로 보여줍니다.

### 서버 알림 보강
- 배고픔/기력 호출도 `isLogged=true`이면 Discord/앱 긴급 알림 대상에서 제외하도록 서버 긴급 이슈 판정을 보강했습니다.
- 수면 조명과 동일하게 현재 대응해야 하는 호출만 외부 알림 대상으로 유지합니다.

### 테스트 보강
- 호출 helper가 현재 호출과 최근 호출을 분리하는지, 처리된 수면 조명 경고 문구가 표시되는지, 호출 팝업 탭과 `🕘📣` 아이콘이 동작하는지 테스트를 추가했습니다.
- 서버 알림 테스트에 `isLogged=true`인 배고픔/기력 호출이 알림으로 생성되지 않는 회귀 케이스를 추가했습니다.
