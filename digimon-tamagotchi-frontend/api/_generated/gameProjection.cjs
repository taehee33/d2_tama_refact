    ...
    evolutions: [
      {
        ...
        conditionGroups: [
          ...
          { careMistakes: { max: 3 }, trainings: { min: 8, max: 23 }, overfeeds: { min: 4 } },
          { careMistakes: { max: 3 }, trainings: { min: 8, max: 23 }, sleepDisturbances: { max: 2 } },
        ],
      },
      {
        targetId: "Mojyamon",
...
function getEnergyRecoverySleepStatus(stats = {}, sleepSchedule, timestampMs) {
  ...
  const fastSleepStartMs = toTimestamp(stats.fastSleepStart) ?? (
    napUntilMs == null ? null : napUntilMs - energyRecovery_NAP_DURATION_MS - energyRecovery_FALLING_ASLEEP_DELAY_MS
  );
  if (fastSleepStartMs != null && napUntilMs != null && timestampMs < napUntilMs) {
    return timestampMs < fastSleepStartMs + energyRecovery_FALLING_ASLEEP_DELAY_MS
      ? "FALLING_ASLEEP"
      : "NAPPING";
  }
...
function stats_isSleepLikeStatus(status) {
  return (
    status === "NAPPING" ||
    status === "SLEEPING" ||
    status === "SLEEPING_LIGHT_ON"
...
function analyzeSleepStatesInRange(stats = {}, startTime, endTime, sleepSchedule) {
  ...
  while (current < actualEnd) {
    ...
    if (stats_isSleepLikeStatus(status)) {
 ...
 */
function getRealtimeArenaActionMatchupResult(action, opponentAction) {
...
function createRealtimeArenaCpuCandidates(rules) {
...
function selectRealtimeArenaCpuOpponent({ host, rules, seed, candidates = createRealtimeArenaCpuCandidates(rules) }) {
...
function selectRealtimeArenaCpuAction({ seed, battleId, round, currentHp, participants }) {
