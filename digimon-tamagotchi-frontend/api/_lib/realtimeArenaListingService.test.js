...
test("대기방 목록은 만료·참가 완료 방과 UID를 제외하고 최신순으로 반환한다", async () => {
  const docs = [
    document("old", { hostUid: "host-1", guestUid: null, status: "waiting", createdAt: new Date("2026-07-30T00:00:00Z"), expiresAt: new Date("2026-07-30T00:20:00Z"), listing: { ownerDisplayName: "테이머 A", hostDigimonName: "아구몬", stage: "Child" } }),
  ...
  assert.equal(rooms[0].ownerDisplayName, "테이머 B");
  assert.equal(JSON.stringify(rooms).includes("hostUid"), false);
  assert.equal(JSON.stringify(rooms).includes("레오몬"), false);
  assert.equal(JSON.stringify(rooms).includes("아구몬"), false);
  assert.equal(Object.hasOwn(rooms[0], "digimonName"), false);
  assert.equal(Object.hasOwn(rooms[0], "stage"), false);
});
