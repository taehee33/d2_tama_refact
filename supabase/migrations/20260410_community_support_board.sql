...
alter table public.community_posts
  ...
  check (
    (
      ...
      slot_id is null and
      snapshot is null and
      category in ('bug', 'question', 'solved')
    )
  );

comment on column public.community_posts.category is
  '텍스트 게시판 말머리. showcase는 null, free는 general/question/guide, support는 bug/question/solved를 사용한다.';

comment on column public.community_posts.image_path is
  '텍스트 게시판 첨부 이미지의 storage 경로. showcase는 null, free/support는 선택적으로 1장만 사용한다.';
