...
이 문서를 읽기 전에, 현재 운영 계약을 먼저 짧게 정리하면 아래와 같다.

- 플레이하려면 로그인해야 한다.
- 로그인 방식은 `Google 로그인` 또는 `게스트(익명) 로그인`이다.
- 게스트 로그인도 현재는 `Firebase Auth`를 사용한다.
- 슬롯의 공식 저장소는 `Firestore`다.
- `localStorage`는 현재 주로 UI 설정, 개발자 옵션, 일부 보조 값에 사용된다.
- `src/repositories`는 남아 있지만 현재 메인 런타임 저장 경계는 아니다.

현재 인증/저장 계약을 더 자세히 보려면 아래 문서를 함께 읽는 것을 권장한다.
...
또한 중요한 점은, 현재 `게스트 로그인`도 Firebase Auth의 익명 로그인이라는 점이다.  
즉 "게스트 = localStorage 전용 비로그인 모드"로 이해하면 현재 코드와 어긋난다.
...
### 현재 localStorage의 실제 의미

현재 localStorage는 "공식 슬롯 저장소"라기보다 아래 역할에 가깝다.
...
즉, 현재 구조를 설명할 때 localStorage를 Firestore와 동급의 메인 슬롯 저장소처럼 설명하면 오해가 생긴다.
