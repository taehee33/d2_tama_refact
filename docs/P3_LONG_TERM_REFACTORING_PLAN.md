...
- `D2_LOCAL_ARCHIVE/sprite-inbox`와 별도 `백업/이미지삭제` 경로에 보존된 외부 보관본은 D2 분류 전 추가 이동·이름 변경·삭제하지 않는다. 저장소에는 해당 후보를 위한 `.gitignore` 규칙을 추가하지 않는다.
...
- 저장소의 `Ver3_Mod_TH`, `Ver4_Mod_codex_48`, `Ver5_Mod_codex_48`에는 현재 미추적 후보 이미지가 없음을 먼저 확인한다.
- 외부 `D2_LOCAL_ARCHIVE/sprite-inbox/INVENTORY.tsv`와 `SHA256SUMS.txt`를 후보 inventory 기준으로 사용하고, 저장소의 추적 자산과 함께 `raw 승인`, `inbox 후보`, `중간 결과`, `배포 결과` 중 하나로 분류한다.
- 외부에 이중 보관된 후보 25개는 사용자 확인 전 추가 이동·이름 변경·삭제하거나 빌드 입력으로 연결하지 않는다.
...
| 원본 이미지 유실/오추적 | `D2_LOCAL_ARCHIVE/sprite-inbox`와 별도 `백업/이미지삭제` 경로에 이중 보관하고 SHA-256 inventory를 검증했으며, D2 분류 전 외부 보관본 변경과 저장소 승격을 금지 |
