...
- **Lease와 플레이 허용:** 검증된 plan만 5분 lease를 만들고 active owner는 대기, stale owner는 transaction으로 인수한다. 오래된 슬롯의 상태 문자열만으로 차단하지 않고 lease를 다시 확인한다. Firestore commit 성공 즉시 `verified`와 gameplay 허용을 확정하며 이후 IndexedDB cleanup 실패는 비차단 저장 경고만 남긴다.
- **Firestore Rules:** `eventId: null`은 `source: legacy_recovery`, replay metadata, ordinal `1..400`을 모두 만족하는 reconciliation incident에만 허용했다. 일반 care incident와 직접 projection 쓰기 제한은 유지했다.
...
- **검증:** 핵심 회귀 7 suites·121 tests 통과. 전체 프런트 220 suites·1,482 tests, 서버 288 tests(266 pass·22 Emulator-only skip), lint, JSDoc typecheck, API 단일 원본 검사와 production build 통과. `npm run test:firestore-emulator` 4/4, `npm run test:arena-emulator` 26/26 통과. `npm run check`는 모든 실행 단계를 통과한 뒤 갱신된 server projection bundle이 아직 커밋되지 않았음을 감지하는 최종 cleanliness 검사에서만 종료되었고, 별도 재생성 전후 checksum으로 생성물 최신 상태를 확인했다.
