...
## [2026-03-10] 에너지 Lazy Update 통합 및 수면 후 회복 보정

### 작업 유형
- 🐛 버그 수정 + ♻️ 리팩토링 (에너지 회복 Lazy Update 통합)

### 목적 및 영향
- **목적:** 수면 후 또는 앱을 오랫동안 켜두지 않았을 때 다시 접속하면 에너지(Energy)가 기대만큼 회복되지 않던 문제를 해결하고, 온라인 타이머와 오프라인 Lazy Update 사이의 에너지 회복 규칙을 통일.
- **문제 원인:** Firebase 슬롯 로드/액션 전에 사용하는 `data/stats.js`의 `applyLazyUpdate`는 배고픔·힘·배변·사망·케어미스만 처리하고, 에너지 회복은 전혀 적용하지 않음. 에너지 Lazy Update 로직은 `logic/stats/stats.js`의 `applyLazyUpdate`에만 존재하지만 실제로는 사용되지 않아, 앱이 꺼져 있던 동안의 수면/경과 시간에 따른 에너지 회복이 반영되지 않고 마지막 저장 시점 값 그대로 남는 구조였음.

### 구현 요약
- **에너지 Lazy Update 전용 함수 도입**
  - `logic/stats/stats.js`에 `applyEnergyLazyUpdate(stats, lastSaved, now, sleepSchedule, maxEnergy)`를 추가.
  - 역할: 마지막 저장 시각부터 현재까지의 시간과 수면 스케줄을 기준으로
    - (1) 수면 종료 후 처음 기상 시점에 `energy = maxEnergy`로 한 번에 풀 회복,
    - (2) 정각/30분마다 `+1` 씩 회복하며 `lastEnergyRecoveryAt` 갱신.
  - 기존 `logic/stats/stats.js`의 `applyLazyUpdate`는 에너지 관련 블록을 제거하고, 내부에서 `applyEnergyLazyUpdate`를 호출하도록 변경해 **에너지 회복 규칙을 한 곳에서 정의**하도록 정리.

- **데이터 계층 Lazy Update에 에너지 회복 적용**
  - `data/stats.js`에서 `applyEnergyLazyUpdate`를 import 한 뒤, `applyLazyUpdate` 내부에서 hunger/strength/poop/사망/케어미스 계산 후:
    - `if (sleepSchedule && maxEnergy) { updatedStats = applyEnergyLazyUpdate(updatedStats, lastSaved, now, sleepSchedule, maxEnergy); }` 를 호출.
  - `useGameData.js`의 `applyLazyUpdateForAction`·슬롯 로드 시 Lazy Update 경로는 기존처럼 `sleepSchedule`과 `maxEnergy`를 계산해 `applyLazyUpdate(stats, lastSavedAt, sleepSchedule, maxEnergy)`를 호출하므로, **Firebase 슬롯 로드/액션 직전에 에너지 회복도 함께 catch-up** 되도록 변경.

### 결과 및 기대 동작
- 앱을 켜 두고 있을 때:
  - 기존과 동일하게 `pages/Game.jsx`의 1초 타이머가 `handleEnergyRecovery`를 호출해 수면 스케줄 기준으로 기상 시 풀 회복, 30분마다 +1 회복을 처리 (동작 변화 없음).
- 앱을 종료했다가 다시 켰을 때:
  - 슬롯 로드 시 `data/stats.js`의 `applyLazyUpdate`가 호출되면서, 마지막 저장 시각 ~ 현재 시각 사이의 수면/시간 경과를 기준으로 에너지가 **미리 계산되어 반영**됨.
  - 예: 밤에 수면 상태로 두고 브라우저를 닫았다가 아침에 다시 접속하면, 해당 기간 동안의 기상·30분 단위 회복이 적용된 상태로 `Energy (Current)`가 표시됨.
- `StatsPopup`의 설명(“기상 시간 회복 (max)”, “30분마다 회복 (+1)”)은 이제 **온라인 타이머와 오프라인 Lazy Update 모두 동일한 규칙**으로 동작하므로, 사용자 체감과 실제 값이 일치함.
...
## [2026-03-09] 덱 배틀 UI 개편 및 VS CPU 모드

### 작업 유형
- ✨ 기능 추가 및 UX 개선 (카드 선택 팝업 제거·일체형 레이아웃, 이번 라운드만 액션 재생, 디지몬 옆 낸 카드 표시, 드로우 애니메이션, 덱 배틀 vs CPU 모드)

### 목적 및 영향
- **목적:** (1) 카드 선택을 팝업이 아닌 배틀 화면 직하단에 통합(상단 상대 / 하단 나 / 맨 아래 손패+덱+사용카드+확인). (2) 덱 배틀 시 “이번 라운드 로그만” 한 번 재생해 카드 1장당 1회 액션으로 정리. (3) 상대/내 디지몬 옆에 이번 라운드에 낸 카드 표시. (4) 시작 시 3→4장·라운드마다 1장 드로우 시각 연출. (5) 덱 배틀 vs CPU 모드 진입 및 로컬 게임 루프 추가.
- **구현 요약:**
  - **BattleScreen.jsx:** `deckVsCpuState` prop 추가. 카드 선택 전용 팝업 제거. `isDeckBattleLayout`(realtime 덱 배틀 또는 deckVsCpu)일 때 배틀 영역 직하단에 `deck-battle-bar`로 손패·확인·덱·사용한 카드 배치. 실시간/VS CPU 공통 `deckBattleRoundLogs`로 “이번 라운드” 로그만 재생(`deckBattleRoundLogIndex`). 상대/내 디지몬 옆에 `lastRound*CardId`·`lastMyCardId`/`lastCpuCardId` 뱃지 표시. 드로우 시 `isDrawAnimating` 및 `animate-deck-draw` CSS 연출. VS CPU 시 “배틀 시작” 단계(손패 3장 + 버튼) 후 1장 드로우하여 choice 단계. `onRoundAnimDone` 호출로 라운드 종료 후 다음 라운드 진행.
  - **useRealtimeBattle.js:** `lastRoundLogEntries` 상태 추가. `round` 수신 시 변환된 로그를 `setLastRoundLogEntries`로 저장하고 `deckBattle.lastRoundLogEntries`로 노출.
  - **useDeckVsCpu.js** (신규): `myDeck`/`myDigimon`/`myStats`, 옵션 `cpuDeck`/`cpuDigimon`/`cpuStats`. `createDeckState`·`draw(3)` 초기화, phase `start` → “배틀 시작” 시 `draw(1)` each → `choice`. 플레이어 선택 시 CPU는 `pickRandomFromHand`, `resolveDeckRound` 호출 후 `roundLogEntries`·히트 반영·`playCardFromHand` 적용, phase `battle_anim`. `onRoundAnimDone`에서 3히트 시 승자·종료, 아니면 `round_done` 후 2초 뒤 다음 라운드. `enemyData`(CPU 디지몬) 및 `startBattle`/`sendChoice`/`onRoundAnimDone` 노출.
  - **BattleSelectionModal.jsx:** “덱 배틀 (vs CPU)” 버튼 및 `onDeckVsCpuStart` 콜백 추가.
  - **Game.jsx:** `useDeckVsCpu` 호출(현재 슬롯 덱·디지몬·스탯), `handlers.onDeckVsCpuStart`, `data.deckVsCpuState` 전달.
  - **GameModals.jsx:** BattleScreen 표시 조건에 `battleType === 'deckVsCpu'` 추가, `deckVsCpuState={data?.deckVsCpuState}` 및 BattleSelectionModal에 `onDeckVsCpuStart` 전달.
  - **Battle.css:** `@keyframes deck-draw-slide`, `.animate-deck-draw` (덱→손패 슬라이드 연출).

### 영향받은 파일
- `digimon-tamagotchi-frontend/src/components/BattleScreen.jsx`
- `digimon-tamagotchi-frontend/src/hooks/useRealtimeBattle.js`
- `digimon-tamagotchi-frontend/src/hooks/useDeckVsCpu.js` (신규)
...
## [2026-03-09] 덱 배틀 새 게임 엔진: 손패/드로우/사용카드 더미 도입

### 작업 유형
- ✨ 동작 변경 (덱 배틀을 손패 기반 플로우로 전면 교체 — 시작 3장 드로우, 라운드 시작 시 1장 드로우, 손패에서 1장 선택 → 공개 → 1회 액션 → 사용한 카드는 덱 위 사용카드 목록으로)

### 목적 및 영향
- **목적:** 덱 배틀을 “덱 10장 + 게임 시작 시 3장 드로우 + 라운드 1 시작 시 1장 드로우 + 손패에서 1장 선택 → 양쪽 공개 → 디지몬 딱 한 번 액션 → 사용한 카드는 덱 위 사용카드 목록”으로 동작하는 완전히 새로운 게임 엔진으로 교체.
- **구현 요약:**
  - **deckBattleEngine.js:** `createDeckState(deckArray)`, `draw(state, n)`, `playCardFromHand(state, cardId)`, `pickRandomFromHand(hand)` 추가. 상태 타입은 `{ deck, hand, usedPile }`. 기존 `resolveDeckRound`, `resolveDeckBattleWinner`, `getRemainingCards`, `pickRandomFrom` 유지(호환용).
  - **useRealtimeBattle.js:** 덱 모드에서 `deckBattleDeck`/`deckBattleHand`/`deckBattleUsedPile` 상태 및 `deckStateRef`·`deckBattleInitializedRef` 도입. `request_choice` 수신 시 라운드 1이면 3+1 드로우로 초기화, 이후엔 1장 드로우. `round` 수신 시 `playCardFromHand`로 손패/사용더미 갱신. 호스트 루프에서도 동일하게 라운드 시작 시 드로우, 라운드 종료 후 playCardFromHand 적용. 타임아웃 시 `pickRandomFromHand(hand)` 사용. `deckBattle` 객체에 `hand`, `deck`, `usedPile`, `deckLength`, `usedCardIds`(usedPile과 동일) 노출.
  - **BattleScreen.jsx:** 덱 배틀 카드 선택 UI를 손패(hand) 기반으로 변경. 오른쪽에 “덱 N장” + “사용한 카드” 목록 표시. 배틀 화면(플레이 중)에서도 실시간 덱 배틀일 때 오른쪽에 덱 장수·사용한 카드 미리보기 표시.

### 영향받은 파일
- `digimon-tamagotchi-frontend/src/logic/battle/deckBattleEngine.js`
