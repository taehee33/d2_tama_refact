...
- **검증·배포:** 수정 전 새 슬롯이 비정상 경과시간과 여러 건의 허위 수면 케어미스를 만드는 테스트, 진화 전 기록 1건이 현재 단계 UI에 노출되는 테스트를 각각 실패시켰다. 수정 후 집중 6 suite·154 test와 전체 프런트 219 suite·1,460 test, 서버 287 test, lint·typecheck·production build가 통과했다. Vercel deployment `dpl_54o3hvNvWoApdepvSixi7TPzphjD`를 운영 alias `dthama.vercel.app`에 반영했고 `main.d0a939ae.js`·HTTP 200을 확인했다. 작업 시작 전부터 수정된 서버 projection 생성물이 미커밋 상태라 `check:server-projection`의 git-diff 검사만 녹색으로 완료하지 못했다.
...
- **읽기 전용 운영 감사:** `care-mistake:audit`은 슬롯·전체 로그·incident를 읽고 `writesPerformed: 0`을 보고한다. `d2tamarefact`의 동일 계정 슬롯 4·5를 실측한 결과, 슬롯 4는 현재 stage 발생 5건·해소 0건·저장 0, 슬롯 5는 발생 2건·해소 0건·저장 0으로 둘 다 `verified`였다. 운영 문서는 변경하지 않았다.
- **운영 배포·복구:** Firestore incident 인덱스, Vercel 운영 코드, Firestore Rules 순서로 `d2tamarefact`/`dthama.vercel.app`에 반영했다. Vercel deployment `dpl_BdXcNB67pD8u13m8jUkLrL8pjSrb`가 READY 및 운영 alias로 확정됐고, 운영 번들 `main.38f28035.js`와 HTTP 200을 확인했다. 복구 직전에 두 슬롯의 identity·revision·전체 로그·기존 incident를 재감사한 뒤, update-time precondition을 걸어 두 슬롯을 하나의 원자 commit으로 복구했다. 결과는 슬롯 4 `revision 48 / careMistakes 5 / unresolved incident 5`, 슬롯 5 `revision 7 / careMistakes 2 / unresolved incident 2`이며, 루트·중첩 projection, checksum, reconciliation receipt fingerprint가 모두 일치했다.
...
- **배포 후 24시간 관찰(2026-08-26):** Firebase 사용량 화면의 8월 25일~26일 구간에서 전체 읽기 약 4,300건, 쓰기 170건을 확인했다. 배포 전 전체 읽기 59,925건과 비교하면 약 55,625건, 92.8% 감소했으며 목표인 전체 15,000건 미만을 달성했다. 전체 읽기 자체가 5,000건 미만이므로 `notifications` 읽기도 논리적으로 5,000건 미만이다. 이 값은 Firebase Usage Dashboard 기준의 반올림 집계이며 컬렉션별 정확한 기여도와 최종 청구량은 각각 Usage Insights와 Billing report에서 구분해 확인한다.
...
- **근본 원인:** `769px`부터 데스크톱 헤더의 3열 Grid가 강제되면서 아이패드의 브랜드, 다수의 메뉴 링크, 채팅·알림·계정 액션의 최소 폭을 수용하지 못하고 서로 겹쳤다.
- **영향 파일:** `digimon-tamagotchi-frontend/src/index.css`
- **아키텍처 결정 근거:** 기존 `MobileTabBar`와 메뉴 정의를 재사용해 라우트·저장 계약·React 상태 로직을 변경하지 않고, 태블릿 헤더의 폭 충돌만 반응형 CSS 경계에서 해결했다. 데스크톱 본문 Grid는 그대로 유지한다.
- **검증:** `NavigationLinks.test.jsx`, `TopNavigation.test.jsx`, `npm run check`, `npm run build:frontend`를 실행한다.

## [2026-08-09] Ver.4·Ver.5 사망 후 새로운 시작 버튼 복구

- **내용:** Ver.4·Ver.5 디지몬이 사망 확인 후 오하카다몬으로 전환되면 `새로운 시작` 버튼이 계속 표시되도록 수정했다. 사망 팝업도 동일한 버전의 사망 폼을 새로운 시작 대상으로 인식한다.
- **근본 원인:** 버전 설정에는 Ver.4·Ver.5 사망 폼 ID가 있었지만, 후속 액션 목록은 Ver.1~Ver.3 ID를 별도로 하드코딩하고 있어 환생 직후 `isDead`가 해제되면 버튼 조건이 모두 거짓이 됐다.
- **영향 파일:** `digimon-tamagotchi-frontend/src/hooks/game-runtime/gameAnimationViewModel.js`, `digimon-tamagotchi-frontend/src/components/DeathPopup.jsx`, 관련 사망 폼·팝업 회귀 테스트.
- **아키텍처 결정 근거:** 사망 폼 목록을 `digimonVersionUtils`의 버전별 설정에서 생성해 새 버전 추가 시 화면 버튼과 액션 흐름이 분리된 목록을 놓치지 않도록 했다. 저장 경계와 환생 데이터 구조는 변경하지 않았다.
- **검증:** 사망 폼 목록과 Ver.4·Ver.5 팝업 환생 분기 회귀 테스트를 추가했고 관련 테스트 28개가 통과했다.
...
- **동작 보존:** 상단 `홈` 메뉴와 모바일 홈 탭은 실제 홈 화면을 여는 `/home` 경로를 계속 사용한다.
- **영향 파일:** `digimon-tamagotchi-frontend/src/components/layout/TopNavigation.jsx`, `digimon-tamagotchi-frontend/src/components/layout/NavigationLinks.test.jsx`.
- **아키텍처 결정 근거:** 로고 클릭은 앱의 기본 진입점, 명시적 홈 메뉴는 실제 홈 화면으로 역할을 분리해 개인별 시작 화면 설정과 일반 홈 이동을 모두 보존한다.

## [2026-08-04] 기본 진입과 명시적 홈 경로 분리

- **내용:** 기본 화면 설정을 따르는 루트(`/`)와 항상 홈을 표시하는 인증 전용 `/home` 경로를 분리했다. 기본 화면이 `플레이`여도 홈 버튼은 `/home`으로 이동한다.
- **네비게이션:** 로그인 사용자의 데스크톱·모바일 홈 링크, 랜딩 CTA, 노트북 복귀 링크가 공통 `HOME_ROUTE`를 사용하도록 연결했다. 비로그인 공개 이동과 로그아웃 후 랜딩 복귀는 기존 루트 계약을 유지한다.
- **영향 파일:** `digimon-tamagotchi-frontend/src/App.jsx`, `digimon-tamagotchi-frontend/src/data/headerNavigation.js`, `digimon-tamagotchi-frontend/src/components/layout/TopNavigation.jsx`, 관련 네비게이션·랜딩·노트북 테스트.
- **아키텍처 결정 근거:** `/`를 기본 시작 화면 결정 전용으로 남기고 사용자가 명시적으로 선택한 홈 이동은 별도 경로로 처리해, 기본 설정 리디렉션과 일반 내비게이션이 서로 간섭하지 않도록 했다.
...
- **동작 보존:** 슬롯 순서 변경 모달, 저장 로직, 보기 방식 localStorage 저장 키는 그대로 유지하고 표시 위치와 버튼 문구만 조정했다.
- **영향 파일:** `digimon-tamagotchi-frontend/src/pages/PlayHub.jsx`, `digimon-tamagotchi-frontend/src/pages/PlayHub.test.jsx`, `digimon-tamagotchi-frontend/src/index.css`.
...
- **영향 파일:**
  - `digimon-tamagotchi-frontend/src/components/realtime-arena/RealtimeArenaScreen.jsx`
  - `digimon-tamagotchi-frontend/src/styles/RealtimeArenaBattle.css`
  - `docs/REFACTORING_LOG.md`
- **아키텍처 결정 근거:** 실시간 배틀 상태·저장·타이머 로직은 변경하지 않고, 기존 모달을 헤더와 스크롤 가능한 본문으로만 나눴다. 모바일의 작은 뷰포트에서도 콘텐츠를 잘라내지 않도록 `dvh`와 safe-area를 사용하고, 데스크톱에서는 기존 중앙 정렬을 유지했다.
...
- **영향 파일:** `RealtimeArenaBattleBoard.jsx`, `RealtimeArenaBattle.css`, `RealtimeArenaComponents.test.jsx`.
- **아키텍처 결정 근거:** 기존 판정 데이터의 `DamageTaken` 의미를 그대로 유지하면서 한 문장에 섞여 있던 참가자별 행동과 피해량을 시각적으로 분리한다.

---

## [2026-08-02] 개발 프론트-구버전 운영 API 실시간 아레나 하위 호환

- **내용:** 개발 프론트에서 `resolve-timeout.round`를 아직 받지 못하는 구버전 운영 API를 사용해야 할 때, 해당 API가 정확히 구버전 오류 메시지를 반환하면 기존 판정 명령을 한 번 재시도한다.
- **이유:** 로컬호스트가 Vercel 운영 API를 프록시하는 구조에서 프론트엔드-서버 계약 버전 차이로 0초 판정이 막히는 문제를 줄이기 위해서다.
- **영향 파일:** `digimon-tamagotchi-frontend/src/hooks/useRealtimeArenaSession.js`, `digimon-tamagotchi-frontend/src/hooks/useRealtimeArenaSession.test.js`.
- **아키텍처 결정 근거:** 새 API에는 대상 라운드를 계속 전송해 멱등성을 유지하고, 구버전 API의 정확한 계약 오류에만 같은 요청 ID로 제한된 하위 호환 재시도를 적용한다.

---

## [2026-08-02] 실시간 아레나 최근 라운드 결과 배치 조정

- **내용:** 최근 라운드의 양쪽 행동과 피해 결과를 남은 시간 안내 및 공격·방어·특수공격 선택 버튼 아래로 이동했다. 현재 선택 현황은 기존처럼 전투 장면 바로 아래에 고정한다.
- **영향 파일:** `RealtimeArenaBattleBoard.jsx`, `RealtimeArenaComponents.test.jsx`.
- **아키텍처 결정 근거:** 서버 상태와 판정 로직은 변경하지 않고 화면의 읽기 순서만 `현재 선택 현황 → 다음 행동 선택 → 이전 라운드 결과`로 정리한다.

---

## [2026-08-02] 실시간 아레나 마감 판정 경쟁·개발 오류 오버레이 수정

- **내용:** 같은 라운드의 `resolve-timeout` 요청이 동시에 들어와 한 요청이 다음 라운드로 전이한 경우, 뒤늦게 재시도된 요청은 새 라운드의 deadline을 검사하지 않고 기존 판정 결과를 멱등 재생한다. 클라이언트는 마감 명령에 대상 `round`를 포함한다.
- **화면 복구:** 자동 복구·마감 판정과 버튼 이벤트의 Promise 실패는 세션 오류 문구로만 표시하고 처리된 예외를 다시 전역으로 전파하지 않아 React 개발 오류 오버레이가 게임 화면을 덮지 않게 했다.
- **영향 파일:** `realtimeArenaHandlers.js`, `realtimeArenaRoundService.js`, `useRealtimeArenaSession.js`, `RealtimeArenaScreen.jsx`, `tests/realtime-arena-emulator.test.js`.
- **아키텍처 결정 근거:** Firestore 트랜잭션의 충돌 재시도는 정상 흐름이므로 명령을 `battleId + round`에 묶어 이미 완료된 판정을 재생한다. 서버 오류는 기존 세션 alert에 남기되 UI 이벤트 Promise의 거부를 전역 오류로 중복 노출하지 않는다.

---

## [2026-08-02] 실시간 아레나 선택 색상·전투 정보 밀도 개선

- **내용:** 행동 현황에서 내 선택은 파란색, CPU/상대 선택은 보라색으로 구분했다. 남은 시간과 자동 선택 안내는 공격·방어·특수공격 버튼 위로 이동해 마감 정보를 먼저 읽도록 했다.
- **전투 정보:** 각 디지몬의 단계 옆에 서버 snapshot의 `sourcePower`와 속성을 표시하고, 상대와의 백신 > 바이러스 > 데이터 > 백신 순환 관계를 `유리/불리/중립`으로 계산해 색상과 문구로 함께 전달한다.
- **영향 파일:** `RealtimeArenaBattleBoard.jsx`, `RealtimeArenaActionPanel.jsx`, `RealtimeArenaBattle.css`, 관련 컴포넌트 테스트.
- **아키텍처 결정 근거:** 파워나 속성을 클라이언트에서 재계산하지 않고 서버가 공개한 참가자 snapshot을 표시한다. 속성 관계만 서버 판정과 동일한 순환 규칙으로 파생한다.

---

## [2026-08-02] 개발환경 실시간 배틀 진입 복구

- **내용:** 개발 빌드에서는 별도 환경변수 없이 실시간 배틀 진입 버튼을 활성화한다. 운영 빌드는 기존처럼 `REACT_APP_REALTIME_ARENA_MVP=true`가 명시된 경우에만 노출한다.
- **로컬 실행:** 기존 `npm start` 개발 서버가 `/api` 요청을 `https://dthama.vercel.app`으로 프록시하도록 구성해 브라우저 CORS 없이 운영과 동일한 서버 판정을 테스트한다.
- **영향 파일:** `src/config/arenaFeatures.js`, 관련 테스트, 프론트엔드 `package.json`.
- **아키텍처 결정 근거:** 운영 feature flag 계약은 유지하면서 로컬 개발에서 `Coming Soon`에 막히거나 CRA가 API 경로를 찾지 못하는 두 문제를 함께 제거한다. 로컬 클라이언트도 실제 Firebase 인증 토큰과 운영 서버 판정 계약을 사용한다.

---

## [2026-08-02] 실시간 아레나 행동 현황·카운트다운 배치 개선

- **내용:** 나와 상대의 행동 현황 영역을 첫 선택 단계부터 항상 같은 높이로 표시하고 전투 장면 아래, 최근 라운드 결과 위로 이동했다. 선택 중에는 내 현재 선택만 표시하고 상대 행동은 `선택 대기`로 유지해 마감 전 비공개 계약을 보존한다.
- **시간 안내:** 라운드 헤더에 있던 남은 시간을 행동 버튼 아래 자동 선택 안내 영역으로 이동했다. 남은 시간이 3초 이하가 되면 배경·테두리·숫자·안내 문구를 함께 빨간색으로 강조한다.
- **영향 파일:** `RealtimeArenaBattleBoard.jsx`, `RealtimeArenaActionPanel.jsx`, `RealtimeArenaBattle.css`, `RealtimeArenaComponents.test.jsx`.
- **아키텍처 결정 근거:** 서버 타이밍과 공개 데이터 계약은 변경하지 않고 기존 `remainingMs`와 참가자 전용 선택 상태를 표현 계층에서만 재배치했다.
...
- **서버 정합성:** CPU 상대와 행동은 public 문서가 아닌 서버 비밀 시드로 결정한다. CPU는 사용자의 현재 선택을 입력받지 않고 라운드 시작 상태만으로 가중 행동을 선택하며, 사용자 제출 또는 시간 초과 요청과 같은 transaction에서 라운드를 판정한다. 생성·행동 응답 유실 뒤 재시도에는 저장된 상대와 라운드 결과를 재생한다.
- **영향 파일:** `digimon-tamagotchi-frontend/src/logic/realtime-arena/cpu.js`, `digimon-tamagotchi-frontend/api/_lib/realtimeArenaCpuService.js`, 기존 realtime API·round service·React UI, 관련 단위/Rules 테스트와 `docs/REALTIME_ARENA_MVP_IMPLEMENTATION_PLAN.md`.
- **아키텍처 결정 근거:** 기존 public/secret Firestore 경로, rules snapshot, 7초 로컬 표시 타이머와 서버 권위 판정을 재사용한다. 선택적 `mode`만 추가하고 누락된 기존 문서는 PvP로 해석해 진행 중 문서와 명령 영수증을 마이그레이션하지 않는다. CPU 경기는 `selecting`으로 바로 생성되므로 대기방 조회 비용과 실시간 Firestore 타이머 쓰기를 추가하지 않는다.
...
- **영향 파일:**
  ...
  - `docs/REFACTORING_LOG.md`
- **아키텍처 결정 근거:** 공용 진행률 계산과 보고 데이터 구조는 유지하고 Discord 메시지 문자열 조립 경계만 변경한다. 인앱 알림, 저장 계약, Firestore 경로와 lazy update 규칙에는 영향이 없다.

---

## [2026-07-26] Discord 일일보고 진행 타임라인 정렬

- **내용:** 일일보고의 시간형 카운터 진행 표시를 `시작 날짜·시각 → 20칸 바 → 데드라인 날짜·시각` 한 줄과 바 중앙의 퍼센트 한 줄로 변경했다. 날짜가 바뀌는 12시간 카운터도 즉시 구분할 수 있도록 KST `M/D HH:mm` 형식을 사용한다.
- **영향 파일:**
  - `digimon-tamagotchi-frontend/api/_lib/notificationReports.js`
  - `tests/api/notificationReports.test.js`
  - `docs/REFACTORING_LOG.md`
- **아키텍처 결정 근거:** 기존 공용 진행률 계산을 재사용하고 일일보고 문자열 조립 경계에서만 날짜·정렬을 적용한다. 긴급 케어, 인앱 알림, 저장 계약과 lazy update 규칙은 변경하지 않는다.
...
- **영향 파일:**
  ...
  - `docs/V3_DATA_AUDIT.md`
- **근거:** Ver.3 데이터가 기존 골격/임시 상태에서 실제 로컬 차트 및 상세 페이지와 다르게 남아 있던 진화 분기와 수치를 정합화하되, 근거가 없는 필드는 임의 보정하지 않는다.
...
- **추가 보정:** 데스크톱 게임 toolbar는 채팅 바로가기/접속자 수, 알림, 테이머명 순서로 정리하고, `테이머:` 접두어를 제거했으며 게임 설정 진입은 테이머 메뉴 안으로 이동했다.
- **영향 파일:**
...
- **영향 파일:**
  - `digimon-tamagotchi-frontend/src/data/stats.js`
- **근거:** countdown별 감소/누적 로직, `activeSeconds > 0` 처리, 수면 정책, 케어미스/부상/사망/진화 흐름은 변경하지 않고 중복된 활동 시간 계산만 한 곳으로 모아 이후 단계별 리팩터링 범위를 작게 유지한다.
...
### 목적 및 영향
- **목적:** 기존 일일 보고처럼 테이머, 긴급 대상 수, 디지몬별 상태와 확인 안내를 Discord에서 빠르게 읽을 수 있게 한다.
- **아키텍처 결정:** Apps Script는 전달만 담당하고 메시지 본문은 기존 서버 pure helper에서 계속 단일 생성한다.
- **영향:** 전송·ack·중복 방지·Firestore 계약은 변경하지 않고 Discord 메시지 형식만 변경한다.

### 검증
- 복수 디지몬과 복수 issue의 Discord Markdown 출력 회귀 테스트
- 알림 API 테스트 및 production build
...
## [2026-06-28] 개발자 도감 공개 옵션 제어 복구

- **내용:** 개발자 모드만으로는 미발견 디지몬을 공개하지 않고, 개발자 모드에서 `도감 미발견 디지몬 공개` 옵션이 켜져 있을 때만 전체 도감을 공개하도록 수정했다. 옵션이 꺼져 있으면 미발견 항목은 기존처럼 `???`, blur, 잠금 상태로 표시한다.
- **영향 파일:**
  - `digimon-tamagotchi-frontend/src/components/panels/EncyclopediaPanel.jsx`
  - `digimon-tamagotchi-frontend/src/components/panels/EncyclopediaPanel.test.jsx`
  - `digimon-tamagotchi-frontend/src/components/SettingsModal.jsx`
...
- **운영 배포:** Supabase `20260719_arena_battle_archive_v2.sql`, Firestore final Rules·인덱스, Vercel `CRON_SECRET`·`REACT_APP_ARENA_GHOST_V2=true`를 반영하고 최종 deployment `dpl_8CaJkfhAtDbc8Lv35wSZkFKVsdRN`을 `https://dthama.vercel.app`에 배포했다. 운영 smoke에서 홈 200, legacy complete/archive 426, 미인증 mirror worker 401, 미인증 Ghost API 401을 확인했다.
- **전역 활성화:** 최종 identity dry-run은 27건 skip·변경 0·오류 0, migration dry-run은 Ghost 14건과 시즌 전적 9건 skip·write 0·오류 0이었다. GitHub Actions와 Vercel의 공유 secret을 교체한 뒤 Production worker 직접 호출에서 두 queue 0건과 archive cleanup 0건을 확인했다. `game_settings/arena_config`를 season 3 조건의 transaction에서 `mode=active`, `minArenaClientSchemaVersion=2`로 전환하고, 직후 Actions run `29918954474`에서 mirror/archive job이 모두 성공했다.
