...
test("긴급 Discord 메시지는 대상 수와 디지몬별 상태를 읽기 쉽게 표시한다", () => {
  ...
  ], "2026. 6. 23. AM 1:00:00", { totalDigimonCount: 3 });
...
test("배고픔과 기력 호출은 deadline 전과 최근 만료 구간에서 긴급 이슈로 만든다", () => {
  ...
  const issues = resolveUrgentIssues({
    ...
    strengthMistakeDeadline: now - 60 * 1000,
  }, {}, now);

  assert.deepEqual(issues.map((issue) => issue.key), ["hunger_call", "strength_call"]);
  assert.equal(issues[0].dedupKey, `hunger_call:${now + 5 * 60 * 1000}`);
  assert.equal(issues[1].dedupKey, `strength_call:${now - 60 * 1000}`);
});
...
test("deadline이 grace window 안에서 지난 배고픔과 기력 호출도 delivery를 만든다", async () => {
  ...
  const store = createStore({
    ...
    digimonStats: createRuntimeStats({
      ...
      hungerMistakeDeadline: now - 2 * 60 * 1000,
      strengthMistakeDeadline: now - 2 * 60 * 1000,
    }),
  });
  ...
  assert.equal(
    [...store.documents.keys()].some((path) => path.startsWith("notification_deliveries/")),
    true
  );
});
...
test("grace window가 지난 오래된 호출은 delivery를 만들지 않는다", async () => {
  ...
  const store = createStore({
    ...
    digimonStats: createRuntimeStats({
      ...
      hungerMistakeDeadline: now - 20 * 60 * 1000,
    }),
  });
...
test("legacy activeIssueKeys는 중복 발송 없이 dedupKey로 정규화한다", async () => {
  ...
  const store = createStore({
    ...
    digimonStats: createRuntimeStats({
      ...
      hungerMistakeDeadline: now + 9 * 60 * 1000,
    }),
  });
  store.documents.set("users/user-1/notificationState/slot1", {
    id: "slot1",
    data: { activeIssueKeys: ["hunger_call"] },
  });
  ...
  assert.equal(payload.reports.length, 0);
  assert.deepEqual(
    store.documents.get("users/user-1/notificationState/slot1").data.activeIssueKeys,
    [`hunger_call:${now + 9 * 60 * 1000}`]
  );
});

test("legacy pending issueKeys는 중복 delivery 생성 없이 dedupKey로 정규화한다", async () => {
  const now = Date.parse("2026-06-21T13:00:00.000Z");
  const store = createStore({
    selectedDigimon: "Agumon",
    isLightsOn: true,
  ...
  store.documents.set("notification_deliveries/existing-delivery", {
  ...
  const deliveryDocuments = [...store.documents.keys()]
    .filter((path) => path.startsWith("notification_deliveries/"));
  assert.equal(payload.summary.newDeliveries, 0);
  assert.equal(payload.summary.reusedDeliveries, 1);
  assert.deepEqual(deliveryDocuments, ["notification_deliveries/existing-delivery"]);
  assert.deepEqual(
    store.documents.get("notification_deliveries/existing-delivery").data.issueKeys,
    [`hunger_call:${now + 9 * 60 * 1000}`]
  );
});
...
test("dryRun은 메시지를 계산하지만 delivery 문서를 만들지 않는다", async () => {
  ...
  assert.match(result.reports[0].messageContent, /👤 \*\*테이머\*\*: 한솔 \(총 1마리\)/);
