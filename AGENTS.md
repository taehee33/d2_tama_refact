...
이 파일은 Codex (Codex.ai/code)가 이 저장소의 코드를 작업할 때 참고하는 가이드입니다.
...
### 단순화 원칙과 보조 리뷰 (Ponytail 선별 적용)

Ponytail의 재사용·과잉 구현 방지 원칙을 프로젝트 지침으로 선별 적용합니다. 로컬 Codex에는 원본 플러그인 `ponytail@ponytail`을 설치하고 기본 모드를 `off`로 설정했습니다. 이 프로젝트에서는 `ponytail-review` 스킬을 보조 리뷰에 사용하며, 일반 코딩용 `ponytail` 스킬이나 `full`·`ultra` 모드는 사용자가 요청할 때만 활성화합니다. 플러그인 설치와 모드 설정은 로컬 환경 설정이며 저장소 복제로 전파되지 않습니다.

- **추가 전에 확인:** 요구사항과 실제 호출 흐름을 읽고, 기존 helper·hook·컴포넌트 → 표준 기능·브라우저 기본 기능 → 설치된 의존성 순서로 해결 가능성을 확인합니다. 요구 동작과 한국어 UI·접근성을 충족할 때 재사용합니다.
- **현재 요구만 구현:** 요청되지 않은 확장성, 범용 설정, 추상화, 새 의존성을 미리 추가하지 않습니다. 명시된 요구사항을 임의로 축소하지 않습니다.
- **책임 경계 우선:** 줄 수·파일 수보다 가독성, 올바른 수정 위치, 유지보수할 복잡성을 기준으로 판단합니다. 호출자가 하나라는 이유만으로 helper·hook을 합치지 않으며, 아래 구조 분리 규칙을 우선합니다.
- **저장 안전성 보존:** Firestore 정본, IndexedDB outbox, identity·revision 검증, transaction, 실패 복구, lazy update 규칙을 단순화 명목으로 축소하지 않습니다. 입력 검증·보안·접근성도 유지합니다.
- **검증·문서화 유지:** 기존 필수 검사와 변경에 필요한 회귀 테스트, 주요 변경 문서화를 유지합니다. 최소 smoke test 하나로 저장 실패·복구·동시성 검증을 대체하지 않습니다.

처음에는 작은 UI 변경 3건에서 변경 diff를 대상으로 다음 보조 리뷰를 수행합니다. 설치된 `ponytail-review` 스킬을 사용하되, 해당 스킬을 사용할 수 없는 환경에서는 이 지침으로 수행하고 그 사실을 기록합니다. 정확성 리뷰와 CI를 대체하지 않습니다.

1. 중복 구현, 불필요한 의존성, 현재 사용하지 않는 확장성을 확인합니다. 레거시 삭제 제안은 실제 참조와 보존 목적을 확인한 뒤 제시합니다.
2. 유효한 발견마다 위치, 단순화 대안, 동작·책임 경계 보존 근거를 짧게 남깁니다. 개선할 부분이 없으면 그대로 마칩니다.
3. 작업 결과에 보조 리뷰의 발견·반영 여부를 간단히 기록합니다. 3건 후 불필요한 제안이나 회귀 여부와 유지보수 개선을 평가하고 적용 범위를 재검토합니다. 삭제 줄 수를 목표로 삼지 않습니다.

참고: [Ponytail](https://github.com/DietrichGebert/ponytail). 외부 원본의 변경을 자동으로 이 지침에 반영하지 않습니다.
